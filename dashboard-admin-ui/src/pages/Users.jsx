...
export default function Users() {
    ...
    const handleViewUser = async (user) => {
        ...
        try {
            ...
            setActionError(getApiErrorMessage(error, 'Không thể tải chi tiết người dùng.'))
        }
    }
    ...
    return (
        <div className={`app-shell processing-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
            ...
            <div className="app-main">
                ...
                <main className="min-w-0 px-4 pb-10 pt-4 sm:px-5 xl:px-6">
                    <header className="mb-4 flex items-start justify-between gap-4">
                        <div><h1 className="text-2xl font-bold tracking-tight text-slate-950 lg:text-[29px]">Quản lý
                            người dùng</h1><p className="mt-1 text-sm text-slate-500">Quản lý tài khoản và quyền truy
                            cập hệ thống.</p></div>
                        <button type="button"
                                className="flex h-10 shrink-0 items-center gap-2 rounded-md bg-blue-600 px-4 text-sm font-semibold text-white shadow-sm hover:bg-blue-700"
                                onClick={openAddForm}><UserPlus size={18}/> Thêm người dùng
                        </button>
                    </header>
                    <div className="space-y-3">
                        {loading && <div
                            className="flex items-center gap-2 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-700">
                            <LoaderCircle size={18} className="animate-spin"/> Đang tải người dùng...</div>}
                        {(loadError || actionError) && <div
                            ...
                            <span className="flex items-center gap-2"><AlertCircle size={18}/>{loadError || actionError}</span>{loadError &&
                            <button type="button" className="font-semibold underline" onClick={loadData}>Thử
                                lại</button>}</div>}
                        <UserSummaryCards users={users}/>
                        <UserFilterBar filters={draftFilters}
                                       officers={[...new Set(users.map((item) => item.officerName).filter(Boolean))]}
                        ...
                        <UserTable users={pageUsers} totalCount={filteredUsers.length} page={page} pageSize={pageSize}
            ...
            <UserDetailPanel user={detailUser} currentUserId={currentUserId} onClose={() => setDetailId(null)}
                             onEdit={() => openEditForm(detailUser.id)}
                             onToggleStatus={() => setStatusTargetId(detailUser.id)}
                             onResetPassword={() => setResetTargetId(detailUser.id)}/>
            <UserFormModal open={Boolean(formMode)} mode={formMode} user={formUser} users={users} officers={officers}
                           departments={departments} currentUserId={currentUserId} saving={saving}
                           apiError={actionError} onClose={() => {
                setFormMode(null);
                setFormUserId(null);
                setActionError('')
            }} onSave={saveUser}/>
            <UserStatusConfirmModal user={statusTarget} onCancel={() => setStatusTargetId(null)}
                                    onConfirm={confirmStatusChange}/>
            <ResetPasswordModal user={resetTarget} loading={saving} error={actionError} onCancel={() => {
                setResetTargetId(null);
                setActionError('')
            }} onConfirm={confirmPasswordReset}/>

            {toast && <div
                className="fixed bottom-5 right-5 z-[100] flex max-w-sm items-center gap-3 rounded-lg border border-emerald-200 bg-white px-4 py-3 text-sm font-semibold text-emerald-700 shadow-xl"
