...
export default function AssignedWork() {
    ...
    const assignerOptions = useMemo(() => {
        ...
        myList.items.forEach((item) => uniqueAssigners.set(item.assignerId, {
            id: item.assignerId,
            name: item.assignerName
        }))
    ...
    return (
        <div className={`app-shell processing-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
            ...
            <div className="app-main">
                ...
                <main className="w-full px-4 pb-10 pt-4 sm:px-5 xl:px-6">
                    <header className="mb-4">
                        <h1 className="text-2xl font-bold tracking-tight text-slate-950 lg:text-[29px]">Việc được
                            giao</h1>
                    ...
                    {loading.my && <div
                        ...
                        <LoaderCircle size={18} className="animate-spin"/> Đang tải danh sách công việc...</div>}
                    {(errors.my || errors.mySummary) && <div
                        ...
                        <span className="flex items-center gap-2"><AlertCircle
                            size={18}/>{errors.my || errors.mySummary}</span>
                    ...
                    {(actionMessage || errors.detail) && <div
                        className="mb-3 flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                        <AlertCircle size={18}/>{actionMessage || errors.detail}</div>}

                    <div
                        className={`grid items-start gap-3 ${detail ? 'xl:grid-cols-[minmax(0,1fr)_420px]' : 'grid-cols-1'}`}>
                        <div className="min-w-0 space-y-3">
                            <AssignedWorkSummary summary={mySummary}/>
                            <AssignedWorkTabs summary={mySummary} activeTab={activeTab} onChange={(tab) => {
                                setPage(1);
                                setActiveTab(tab)
                            }}/>
                            <AssignedWorkFilter
                        ...
                        {loading.detail && !detail
                            ...
                            : <AssignedWorkDetailPanel assignment={detail}
            ...
            <AcceptWorkModal open={acceptModalOpen} submitting={loading.action}
                             onCancel={() => setAcceptModalOpen(false)} onConfirm={confirmAccept}/>
            <RejectWorkModal open={rejectModalOpen} reason={rejectReason} submitting={loading.action}
                             onReasonChange={setRejectReason} onCancel={() => {
                setRejectModalOpen(false);
                setRejectReason('')
