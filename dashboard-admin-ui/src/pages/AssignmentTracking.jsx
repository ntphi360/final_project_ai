    ...
    return (
        <div className={`app-shell processing-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
            ...
            <div className="app-main">
                <Header onNotificationClick={() => setNotificationsOpen((open) => !open)}
                        notificationCount={trackingSummary.accepted + trackingSummary.rejected}/>
                {notificationsOpen && (
                    <section
                        className="fixed right-5 top-[62px] z-[70] w-[min(380px,calc(100vw-24px))] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl"
                        aria-label="Thông báo phản hồi giao việc">
                        <header className="border-b border-slate-200 px-4 py-3 text-sm font-bold text-slate-900">Phản
                            hồi giao việc
                        </header>
                        <div className="max-h-80 overflow-y-auto p-2">
                            {responseNotifications.map((item) => <button type="button"
                                ...
                                className="font-semibold text-slate-900">{item.assigneeName}</span>{item.status === 'ACCEPTED' ? ' đã xác nhận nhận hồ sơ ' : ' đã từ chối hồ sơ '}<span
                                className="font-semibold text-blue-700">{item.caseCode}</span>.</button>)}
                            {responseNotifications.length === 0 &&
                                <p className="px-3 py-5 text-center text-sm text-slate-500">Chưa có phản hồi trên trang
                                    hiện tại.</p>}
                        </div>
                    </section>
                )}
                <main className="px-4 pb-10 pt-4 sm:px-5 xl:px-6">
                    <header className="mb-4">
                        <h1 className="text-2xl font-bold tracking-tight text-slate-950 lg:text-[29px]">Theo dõi giao
                            việc</h1>
                        <p className="mt-1 text-sm text-slate-500">Theo dõi tình trạng tiếp nhận và phản hồi của các
                            công việc đã giao.</p>
                    </header>

                    {loading.tracking && <div
                        ...
                        <LoaderCircle size={18} className="animate-spin"/> Đang tải danh sách giao việc...</div>}
                    {optionsLoading && <div
                        className="mb-3 flex items-center gap-2 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-medium text-blue-700">
                        <LoaderCircle size={18} className="animate-spin"/> Đang tải dữ liệu bộ lọc...</div>}
                    {optionsError && <div
                        ...
                        <span className="flex items-center gap-2"><AlertCircle size={18}/>{optionsError}</span>
                        <button type="button" className="font-semibold underline" onClick={loadOptions}>Thử lại</button>
                    </div>}
                    {(errors.tracking || errors.trackingSummary) && <div
                        className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                        <span className="flex items-center gap-2"><AlertCircle
                            size={18}/>{errors.tracking || errors.trackingSummary}</span>
                    ...
                    {errors.detail && <div
                        ...
                        <AlertCircle size={18}/>{errors.detail}</div>}

                    <div
                        className={`grid items-start gap-3 ${detail ? 'xl:grid-cols-[minmax(0,1fr)_440px]' : 'grid-cols-1'}`}>
                        <div className="min-w-0 space-y-3">
                            <AssignmentTrackingSummary summary={trackingSummary}/>
                            <AssignmentTrackingTabs summary={trackingSummary} activeTab={activeTab} onChange={(tab) => {
                                setPage(1);
                                setActiveTab(tab)
                            }}/>
                        ...
                        {loading.detail && !detail
                            ? <div
                                className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-600">
                                <LoaderCircle size={18} className="animate-spin"/> Đang tải chi tiết...</div>
                            : <AssignmentTrackingDetailPanel assignment={detail}
                                                             onClose={() => dispatch(clearAssignmentDetail())}
