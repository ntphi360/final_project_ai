...
export default function DataImport() {
    ...
    const processImport = async () => {
        ...
        try {
            ...
            const errors = data.error_records > 0
                ? [{
                    row: '—',
                    message: `Backend ghi nhận ${data.error_records} bản ghi lỗi nhưng chưa trả chi tiết từng dòng.`
                }]
                : []
            const importResult = {
    ...
    return (
        <div className={`app-shell processing-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
            ...
            <div className="app-main">
                ...
                <main className="px-4 pb-10 pt-4 sm:px-5 xl:px-6">
                    <header className="mb-4">
                        <h1 className="text-2xl font-bold tracking-tight text-slate-950 lg:text-[29px]">Import dữ
                            liệu</h1>
                    ...
                    <div className="grid items-start gap-4 lg:grid-cols-[minmax(300px,35fr)_minmax(0,65fr)]">
                        <div className="space-y-3">
                            {apiError && <div
                                ...
                                <AlertCircle size={18} className="mt-0.5 shrink-0"/><span>{apiError}</span></div>}
                            <ImportDropzone file={selectedFile} error={fileError} onSelect={validateFile}
                                            onRemove={() => {
                                                setSelectedFile(null);
                                                setFileError('');
                                                setApiError('');
                            ...
                            <button type="button" disabled={!selectedFile || Boolean(fileError) || processing}
                                    className="flex h-11 w-full items-center justify-center gap-2 rounded-md bg-blue-600 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                                    onClick={processImport}>
                                {processing ? <><LoaderCircle size={18} className="animate-spin"/> Đang xử lý...</> : <>
                                    <Play size={18}/> Xử lý dữ liệu</>}
                            </button>
                        ...
                        <div className="space-y-2">
                            {historyError && <div
                                className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-3 text-sm text-red-700">
                                <AlertCircle size={18} className="mt-0.5 shrink-0"/><span>{historyError}</span></div>}
                            {historyLoading ? <div
                                className="flex min-h-40 items-center justify-center rounded-lg border border-slate-200 bg-white text-sm text-slate-500">
                                <LoaderCircle size={18} className="mr-2 animate-spin"/> Đang tải lịch sử import...
                            </div> : <ImportHistoryTable items={history} page={historyPage} pageSize={10}
                                                         onPageChange={setHistoryPage} onView={openHistoryDetail}/>}
                        </div>
                    </div>

                    <section
                        className="mt-4 flex gap-3 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm leading-6 text-slate-600">
                        <Info size={20} className="mt-0.5 shrink-0 text-blue-600"/>
                        <div><p>Sau khi import, hệ thống sẽ tự động kiểm tra dữ liệu trùng và cập nhật.</p><p>Dữ liệu hồ
                            sơ đã hoàn thành sẽ được sử dụng để huấn luyện/đánh giá model khi thực hiện chức năng
                            AI.</p></div>
                    </section>
                </main>
            </div>
            ...
            {detailLoading && <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/15"><span
                className="flex items-center gap-2 rounded-lg bg-white px-4 py-3 text-sm text-slate-600 shadow-xl"><LoaderCircle
                size={18} className="animate-spin"/> Đang tải chi tiết...</span></div>}
            <ImportErrorModal open={Boolean(visibleErrors)} errors={visibleErrors || []}
                              onClose={() => setVisibleErrors(null)}/>

            {toast && <div
                className="fixed bottom-5 right-5 z-[90] flex max-w-sm items-center gap-3 rounded-lg border border-emerald-200 bg-white px-4 py-3 text-sm font-semibold text-emerald-700 shadow-xl"
                role="status"><CheckCircle2 size={20}/><span>{toast}</span>
