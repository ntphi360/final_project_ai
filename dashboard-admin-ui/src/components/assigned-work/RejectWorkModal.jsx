...
export default function RejectWorkModal({ open, reason, submitting = false, onReasonChange, onCancel, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={onCancel}>
      <section className="relative w-full max-w-lg rounded-xl border border-slate-200 bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="reject-work-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-red-50 text-red-600"><XCircle size={25} /></span>
        <h2 id="reject-work-title" className="mt-3 text-center text-lg font-bold text-slate-900">Từ chối công việc</h2>
        <label className="mt-5 block">
          <span className="mb-2 block text-sm font-semibold text-slate-700">Lý do từ chối</span>
          <textarea autoFocus maxLength={500} value={reason} placeholder="Nhập lý do từ chối..." className="min-h-32 w-full resize-y rounded-md border border-slate-200 p-3 text-sm leading-6 outline-none placeholder:text-slate-400 focus:border-red-400 focus:ring-2 focus:ring-red-100" onChange={(event) => onReasonChange(event.target.value)} />
