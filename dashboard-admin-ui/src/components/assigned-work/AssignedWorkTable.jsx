...
export default function AssignedWorkTable({ assignments, totalCount, page, pageSize, onView, onPageChange, onPageSizeChange }) {
      ...
      <div className="w-full overflow-x-auto">
        <table className="min-w-[1080px] table-fixed border-collapse text-sm text-slate-700">
          ...
          <tbody>
            ...
            {assignments.length === 0 && <tr><td className="h-24 text-center text-sm text-slate-500" colSpan="8">Không tìm thấy công việc phù hợp.</td></tr>}
          </tbody>
        </table>
      </div>
      <footer className="flex min-h-14 flex-col gap-3 px-4 py-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center gap-1">
            <button type="button" disabled={page === 1} aria-label="Trang trước" className="grid h-9 w-9 place-items-center rounded-md border border-slate-200 bg-white hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300" onClick={() => onPageChange(page - 1)}><ChevronLeft size={17} /></button>
            {visiblePages(page, totalPages).map((pageNumber) => (
              <button type="button" className={`grid h-9 w-9 place-items-center rounded-md border text-sm ${pageNumber === page ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-200 bg-white hover:bg-slate-50'}`} onClick={() => onPageChange(pageNumber)} key={pageNumber}>{pageNumber}</button>
            ))}
            <button type="button" disabled={page === totalPages} aria-label="Trang sau" className="grid h-9 w-9 place-items-center rounded-md border border-slate-200 bg-white hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300" onClick={() => onPageChange(page + 1)}><ChevronRight size={17} /></button>
          </div>
          <select value={pageSize} aria-label="Số công việc mỗi trang" className="h-9 rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-500" onChange={(event) => onPageSizeChange(Number(event.target.value))}>
