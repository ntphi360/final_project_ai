    ...
    return (
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between gap-3">
                ...
                {result.errorRows > 0 && <button type="button"
                                                 ...
                                                 onClick={onViewErrors}><Eye size={15}/> Xem lỗi</button>}
            </div>
            <div className="grid grid-cols-2 gap-2">
                {items.map(({key, label, icon: Icon, tone}) => (
                    <article className={`rounded-md p-3 ${tone}`} key={key}>
                        <span className="flex items-center gap-1.5 text-xs font-semibold"><Icon
                            size={15}/> {label}</span>
            ...
            <dl className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600">
                <div className="flex justify-between rounded-md bg-slate-50 px-3 py-2">
                    <dt>Thêm mới</dt>
                    <dd className="font-bold text-slate-900">{formatImportNumber(result.insertedRows || 0)}</dd>
                </div>
                <div className="flex justify-between rounded-md bg-slate-50 px-3 py-2">
                    <dt>Cập nhật</dt>
                    <dd className="font-bold text-slate-900">{formatImportNumber(result.updatedRows || 0)}</dd>
                </div>
                <div className="flex justify-between rounded-md bg-slate-50 px-3 py-2">
                    <dt>Chưa ánh xạ</dt>
                    <dd className="font-bold text-slate-900">{formatImportNumber(result.unmappedRows || 0)}</dd>
                </div>
                <div className="flex justify-between rounded-md bg-slate-50 px-3 py-2">
                    <dt>Sai lĩnh vực</dt>
                    <dd className="font-bold text-slate-900">{formatImportNumber(result.fieldMismatchRows || 0)}</dd>
                </div>
            </dl>
        </section>
    )
