...
export default function AssignmentTrackingTable({
                                                    assignments,
                                                    totalCount,
                                                    page,
                                                    pageSize,
            ...
            <div className="w-full overflow-x-auto">
                <table className="min-w-[1120px] table-fixed border-collapse text-sm text-slate-700">
                    ...
                    {assignments.map((item, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className="h-12 border-b border-slate-100 px-3"><span
                                    className="block truncate">{item.assigneeName}</span></td>
                                ...
                                <td className="h-12 border-b border-slate-100 px-3"><span
                                    className="inline-flex rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">{channelLabel(item.notificationChannels)}</span>
                                </td>
                    ...
                    {assignments.length === 0 && <tr>
                        <td className="h-24 text-center text-sm text-slate-500" colSpan="10">Không tìm thấy giao việc
                            phù hợp.
                        </td>
                    </tr>}
                    </tbody>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <div className="flex items-center gap-1">
                        <button type="button" disabled={page === 1} aria-label="Trang trước"
                                className="grid h-9 w-9 place-items-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300"
                    ...
                    <select value={pageSize} aria-label="Số giao việc mỗi trang"
