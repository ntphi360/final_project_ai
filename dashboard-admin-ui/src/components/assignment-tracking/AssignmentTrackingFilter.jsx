...
export default function AssignmentTrackingFilter({ filters, options, onChange, onApply, onReset }) {
  return (
    <form className="grid gap-2 rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm sm:grid-cols-2 xl:grid-cols-5 2xl:grid-cols-[minmax(220px,1.4fr)_repeat(6,minmax(120px,0.8fr))_90px_90px] 2xl:items-end" onSubmit={(event) => { event.preventDefault(); onApply() }}>
      ...
      <button type="submit" className="flex h-10 items-center justify-center gap-2 rounded-md bg-blue-600 px-3 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700"><Filter size={16} /> Lọc</button>
      <button type="button" className="flex h-10 items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50" onClick={onReset}><RotateCcw size={16} /> Đặt lại</button>
    </form>
  )
}
...
function DateFilter({ label, value, onChange }) {
  return (
    <label>
      <span className="mb-1 block text-xs font-medium text-slate-600">{label}</span>
      <input type="date" value={value} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" onChange={(event) => onChange(event.target.value)} />
    </label>
  )
}
