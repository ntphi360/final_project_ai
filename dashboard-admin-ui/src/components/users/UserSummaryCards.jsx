...
export default function UserSummaryCards({users}) {
    ...
    return (
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-4" aria-label="Tổng quan người dùng">
            {cards.map(({id, label, icon: Icon, tone}) => <article
                className="flex min-h-24 items-center gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm"
                key={id}><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${tone}`}><Icon
                size={23}/></span>
                <div><span className="text-sm font-medium text-slate-500">{label}</span><strong
                    className="mt-1 block text-2xl text-slate-950">{counts[id]}</strong></div>
            </article>)}
        </section>
    )
}
