...
export default function UserFormModal({ open, mode, user, users, officers, departments, saving, apiError, onClose, onSave }) {
    ...
    <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/40 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <section className="w-full max-w-2xl overflow-hidden rounded-xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="user-form-title">
        <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <div><h2 id="user-form-title" className="text-lg font-bold text-slate-950">{mode === 'add' ? 'Thêm người dùng' : 'Chỉnh sửa người dùng'}</h2><p className="mt-1 text-xs text-slate-500">{mode === 'add' ? 'Tạo tài khoản mới cho người dùng nội bộ.' : 'Cập nhật thông tin và quyền truy cập.'}</p></div>
          <button type="button" aria-label="Đóng form người dùng" className="grid h-8 w-8 place-items-center rounded-md text-slate-500 hover:bg-slate-100" onClick={onClose}><X size={19} /></button>
        </header>
        <form onSubmit={submit}>
          <div className="grid max-h-[70vh] gap-4 overflow-y-auto p-5 sm:grid-cols-2">
            ...
            <label><span className="mb-1 block text-sm font-medium text-slate-700">Vai trò <b className="text-red-500">*</b></span><select value={form.role} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" onChange={(event) => changeRole(event.target.value)}>{roleOptions.map((role) => <option value={role.value} key={role.value}>{role.label}</option>)}</select><FieldError message={errors.role} /></label>

            {form.role === 'OFFICER' && (
              <label className="sm:col-span-2"><span className="mb-1 block text-sm font-medium text-slate-700">Cán bộ liên kết <b className="text-red-500">*</b></span><select value={form.createOfficer ? '__create__' : form.officerId} className={`h-10 w-full rounded-md border bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-blue-100 ${errors.officerId ? 'border-red-400' : 'border-slate-200 focus:border-blue-500'}`} onChange={(event) => changeOfficerMode(event.target.value)}><option value="">Không áp dụng</option>{availableOfficers.map((officer) => <option value={officer.id} key={officer.id}>{officer.name}</option>)}{mode === 'add' && <option value="__create__">+ Tạo cán bộ mới</option>}</select><FieldError message={errors.officerId} /></label>
            )}

            {form.role === 'OFFICER' && form.createOfficer && (
              <>
                <label className="sm:col-span-2"><span className="mb-1 block text-sm font-medium text-slate-700">Phòng ban <b className="text-red-500">*</b></span><select value={form.departmentId} className={`h-10 w-full rounded-md border bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-blue-100 ${errors.departmentId ? 'border-red-400' : 'border-slate-200 focus:border-blue-500'}`} onChange={(event) => changeDepartment(event.target.value)}><option value="">Chọn phòng ban</option>{departments.map((department) => <option value={department.id} key={department.id}>{department.name}</option>)}</select><FieldError message={errors.departmentId} /></label>
                <fieldset className="sm:col-span-2">
                  <legend className="mb-1 text-sm font-medium text-slate-700">Lĩnh vực phụ trách <b className="text-red-500">*</b></legend>
                  <div className={`max-h-36 overflow-y-auto rounded-md border p-3 ${errors.fieldIds ? 'border-red-400' : 'border-slate-200'}`}>
                    {!form.departmentId && <p className="text-sm text-slate-500">Chọn phòng ban để xem lĩnh vực.</p>}
                    {form.departmentId && fieldsLoading && <p className="text-sm text-slate-500">Đang tải lĩnh vực...</p>}
                    {form.departmentId && !fieldsLoading && !fieldsError && !departmentFields.length && <p className="text-sm text-slate-500">Phòng ban chưa có lĩnh vực hoạt động.</p>}
                    <div className="grid gap-2 sm:grid-cols-2">{departmentFields.map((field) => <label className="flex items-center gap-2 text-sm text-slate-700" key={field.id}><input type="checkbox" checked={form.fieldIds.includes(field.id)} className="h-4 w-4 rounded border-slate-300 text-blue-600" onChange={() => toggleField(field.id)} />{field.name}</label>)}</div>
                  </div>
                  <FieldError message={fieldsError || errors.fieldIds} />
                  <button type="button" className="mt-2 text-xs font-semibold text-blue-600 hover:text-blue-700" onClick={() => changeOfficerMode('')}>Chọn cán bộ có sẵn</button>
                </fieldset>
              </>
            )}
